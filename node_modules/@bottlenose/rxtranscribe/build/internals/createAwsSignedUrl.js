...
function createAwsSignedUrl(method, host, path, service, payload, options = {}) {
  ...
  optionsCopy.headers = options.headers || {}; // host is required
...
const getAwsSignedUrl = function getAwsSignedUrl({
  ...
  const endpoint = `transcribestreaming.${region}.amazonaws.com:8443`; // get a preauthenticated URL that we can use to establish our WebSocket

  return _createAwsSignedUrl('GET', endpoint, '/stream-transcription-websocket', 'transcribe', crypto.createHash('sha256').update('', 'utf8').digest('hex'), {
    region,
    key: accessKeyId,
