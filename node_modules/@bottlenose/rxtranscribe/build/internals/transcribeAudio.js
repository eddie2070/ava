...
import convertAudioToBinaryMessage from './convertAudioToBinaryMessage'; // const transcribe = new AWS.TranscribeService();
...
const transcribeAudio = function transcribeAudio({
  ...
  return source$ => {
    ...
    const message$ = source$.pipe(map(audioBinary => convertAudioToBinaryMessage(audioBinary)));
    const ws$ = url$.pipe(mergeMap(url => _ws({
      url,
      message$
    })));
    const jsonResponse$ = ws$.pipe(_send(message$));
    return jsonResponse$;
  };
};
