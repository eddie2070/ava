...
describe('operators.transcribe', () => {
  it('should properly call its workflow', marbles(m => {
    const input$ = m.cold('-0--1---2|', [Buffer.from('foobar', 'base64'), Buffer.from('anotherfoobar', 'base64'), Buffer.from('lastfoobar', 'base64')]);
    const params = {
      accessKeyId: 'fakeaccesskey',
      secretAccessKey: 'fakesecretkey',
      _conduit: sinon.stub().returns(source$ => source$.pipe(mapTo(JSON.stringify({
        foo: 'some json'
      })))),
      _convertAudioToBinaryMessage: sinon.stub().returns(Buffer.from('message', 'base64')),
      _getPresignedUrl: sinon.stub().returns('wss://buccaneer.ai?something')
    };
    const actual$ = input$.pipe(transcribe(params));
    const expected$ = m.cold('-0--1---2|', [JSON.stringify({
      foo: 'some json'
    }), JSON.stringify({
      foo: 'some json'
    }), JSON.stringify({
