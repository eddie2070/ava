...
import { NEW_MESSAGE } from '../internals/actions'; // the WebSocket emits messages wrapped in an object
...
describe('messages operator', () => {
  it('should emit message objects', marbles(m => {
    ...
    const inputs = {
      ...
      5: ['client', {
        type: NEW_MESSAGE,
        data: formatData(messageArr[2])
      }]
    };
    const input$ = m.cold('0-1(23)4-5|', inputs);
    const actual$ = input$.pipe(messages());
    m.expect(actual$).toBeObservable(m.cold('0--2-----5|', {
      0: messageArr[0],
      2: messageArr[1],
      5: messageArr[2]
    }));
